...
describe('payload approval', () => {
  ...
  it('queues independent runs without rejecting or granting the other payload', async () => {
    ...
    expect(pendingPayloadApproval.value?.hash).toBe('hash')
  ...
  it('cancels a queued review independently and starts its expiry only when displayed', async () => {
    vi.useFakeTimers()
    const first = requestPayloadApproval(request)
    const abort = new AbortController()
    const cancelled = requestPayloadApproval({ ...request, hash: 'cancelled' }, abort.signal)
    const third = requestPayloadApproval({ ...request, hash: 'third' })
    abort.abort()
    expect(await cancelled).toBe(false)
    await vi.advanceTimersByTimeAsync(119_000)
    resolvePayloadApproval(pendingPayloadApproval.value!.id, false)
    expect(await first).toBe(false)
    expect(pendingPayloadApproval.value?.hash).toBe('third')
