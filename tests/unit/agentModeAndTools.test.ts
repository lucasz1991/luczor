...
import {
  buildSystemPreamble,
  looksLikeInternalReasoningLeak,
  runAgent,
...
describe("agent mode and tool reliability", () => {
  ...
  it("recognizes explicit execution requests but not ordinary questions", () => {
    ...
    expect(looksLikeInternalReasoningLeak("We need to respond: The user says hello."))
      .toBe(true);
    expect(looksLikeInternalReasoningLeak("Ich habe den Projektzustand geprüft."))
      .toBe(false);
  });
