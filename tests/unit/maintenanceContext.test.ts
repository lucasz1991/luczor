...
describe('bounded model-requested maintenance evidence', () => {
  it('distinguishes strict local read requests from final proposals', () => {
    expect(parseMaintenanceContextRequest('{"request_context":{"query":"Redis","limit":2}}')).toEqual({
      query: 'Redis',
      limit: 2,
    })
  ...
  it('never crosses account, project, sensitivity, deletion, expiry or derived-memory write boundaries', () => {
    const invalid = [
      ...
      { ...record('deleted'), status: 'superseded' as const },
    ...
    expect(
      select([source, { ...record('other-dataset'), dataset: 'foreign' }], {
        kind: 'memory',
        current: [memoryMaintenanceSource(source)],
      })
    ).toEqual([])
  })
  it('fits the combined budget and never truncates a large source or repeats evidence', () => {
    ...
    expect(
      select(records, { current: Array.from({ length: 6 }, (_, i) => memoryMaintenanceSource(record(`seen-${i}`))) })
    ).toEqual([])
  })
  it('closes the read protocol at its round limit and keeps data explicitly untrusted', () => {
    const prompt = maintenanceContextPrompt(
      'repository',
      [memoryMaintenanceSource(record('a'))],
      0,
