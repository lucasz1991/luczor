...
describe('device job transport startup', () => {
  ...
  it('backs off repeated poll failures, logs a changed failure once and clears the recovered poll error', async () => {
    api.registerDevice.mockImplementation(() => new Promise(() => {}))
    api.nextDeviceJob.mockRejectedValue(new Error('Request timed out after 10000 ms.'))
    const { startDeviceJobChannel, getDeviceJobChannelState } = await import('@/services/deviceJobs')
    stop = await startDeviceJobChannel()
    await vi.advanceTimersByTimeAsync(0)
    expect(api.nextDeviceJob).toHaveBeenCalledTimes(1)
    expect(getDeviceJobChannelState()).toMatchObject({ rest: 'error', lastError: 'Request timed out after 10000 ms.' })
    await vi.advanceTimersByTimeAsync(20000)
    expect(api.nextDeviceJob).toHaveBeenCalledTimes(2)
    ...
    await vi.advanceTimersByTimeAsync(20000)
    expect(api.nextDeviceJob).toHaveBeenCalledTimes(3)
    expect(getDeviceJobChannelState()).toMatchObject({ rest: 'polling', lastError: null })
    await vi.advanceTimersByTimeAsync(20000)
    expect(api.nextDeviceJob).toHaveBeenCalledTimes(4)
  })

  it('retries failed notification catch-up independently of jobs with cooldown and cancellation', async () => {
    api.registerDevice.mockImplementation(() => new Promise(() => {}))
    notifications.catchUp.mockRejectedValue(new Error('notifications timeout'))
    const { startDeviceJobChannel, getDeviceJobChannelState } = await import('@/services/deviceJobs')
    stop = await startDeviceJobChannel()
    await vi.advanceTimersByTimeAsync(20000)
    expect(notifications.catchUp).toHaveBeenCalledTimes(2)
    expect(api.nextDeviceJob).toHaveBeenCalledTimes(2)
    document.dispatchEvent(new Event('visibilitychange'))
    await vi.advanceTimersByTimeAsync(20000)
    expect(notifications.catchUp).toHaveBeenCalledTimes(2)
    expect(console.warn).toHaveBeenCalledTimes(1)
    notifications.catchUp.mockResolvedValue(undefined)
    await vi.advanceTimersByTimeAsync(20000)
    expect(notifications.catchUp).toHaveBeenCalledTimes(3)
    expect(getDeviceJobChannelState().lastError).toBeNull()
    stop()
  ...
  it('starts no background requests offline and immediately recovers on an online event', async () => {
    vi.stubGlobal('navigator', { platform: 'test', onLine: false })
    api.registerDevice.mockImplementation(() => new Promise(() => {}))
    const { startDeviceJobChannel } = await import('@/services/deviceJobs')
    stop = await startDeviceJobChannel()
    await vi.advanceTimersByTimeAsync(60000)
    expect(api.nextDeviceJob).not.toHaveBeenCalled()
    expect(notifications.catchUp).not.toHaveBeenCalled()
    expect(api.registerDevice).not.toHaveBeenCalled()
    vi.stubGlobal('navigator', { platform: 'test', onLine: true })
    window.dispatchEvent(new Event('online'))
    await vi.advanceTimersByTimeAsync(0)
    expect(api.nextDeviceJob).toHaveBeenCalledOnce()
    expect(notifications.catchUp).toHaveBeenCalledOnce()
