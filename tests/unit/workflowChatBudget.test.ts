  ...
  runInNewContext(
    ...
    }).outputText,
    {
      ...
      require(id: string) {
        ...
        if (id === '@/services/tools/workflows')
          return { workflowTools: [{ name: 'workflow_run_cancel', execute: cancel }] }
...
const renderer = Vue.createRenderer<Node, Node>({
  ...
  patchProp(child, key, _old, value) {
    Reflect.set(child.props, key, value)
  },
  setScopeId() {},
})
...
function mount(extra: Record<string, unknown> = {}) {
  const props = Vue.reactive({
    workflows: [{ id: 7, name: 'Owned workflow', runId: 'run-public', status: 'running' }],
    projectId: 'project-1',
    ...extra,
  })
...
beforeEach(() => {
  vi.clearAllMocks()
  read.mockResolvedValue({ data: run() })
...
describe('chat workflow budget and stop controls', () => {
  it('shows measured root counters and the 80 percent warning instead of child consumption', async () => {
    read.mockResolvedValue({
      data: run({
        root_workflow_run_id: 2,
        budgets: { max_executions: 200 },
        budget_state: { executions: 0 },
        root_budget: { id: 2, status: 'running', budgets: { max_executions: 200 }, budget_state: { executions: 170 } },
      }),
    })
    const { root } = mount()
    await flush()
  ...
  it('keeps an acknowledged boundary stop pending while retaining immediate cancellation', async () => {
    ...
    expect(cancel).toHaveBeenCalledWith(
      { workflow_id: 7, run_id: 'run-public' },
  ...
  it('accepts only bound host projections and emits mini actions without direct API calls', async () => {
    const emitted = vi.fn()
    const { root, click } = mount({
      hostOnly: true,
      hostRunsVerified: true,
      hostRuns: [run({ budgets: { max_executions: 200 }, budget_state: { executions: 160 } })],
      onAction: emitted,
    })
    await flush()
    expect(contents(root)).toContain('160 / 200')
    await click('Nach diesem Schritt stoppen')
  ...
  it('does not forward private run payloads or treat a mismatched root as measured', () => {
    const projected = miniWorkflowBudgetSnapshot(
      run({
        output: { private: 'model content' },
        context: { token: 'secret' },
        root_workflow_run_id: 2,
        root_budget: { id: 99, budgets: { max_executions: 200 }, budget_state: { executions: 170 } },
      })
    )
