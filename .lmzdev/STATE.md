...
## 2026-09-17 – Toasts, chat deletion, native drag attempt (Claude Code)
- Toasts: src/services/toast.ts + src/components/ai/ToastHost.vue mounted in App.vue; voice-idle notice/error and read-aloud failures now toast instead of a manual-dismiss bar above the composer.
- Chat deletion: mutations.deleteConversation (soft-delete via archivedAt) + trash button in SidebarNav chat rows + confirm dialog. Fixed a real race: archiving the only/active chat used to get silently undone by migrateConversations()'s safety net; now the replacement chat is created/selected before the old one is archived.
- Native nudge vertical drag: layout() (MiniChatSurface.vue) no longer resizes/repositions the native window while a drag pointer is down, to stop resize()'s unconditional set_position() from fighting start_dragging(); re-syncs once the drag ends. Root-caused via code reading only — NOT yet verified on a real Tauri build; user reported it still didn't work before this fix, needs retest.
- Verified: vue-tsc/eslint/prettier clean; chatConversations.test.ts green; toolRegistry.test.ts pre-existing 2 failures unrelated (confirmed via git stash). Browser-verified toast + delete flows end to end.
