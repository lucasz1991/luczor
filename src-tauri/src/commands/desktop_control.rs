...
impl Default for DesktopControlConfig {
    fn default() -> Self {
        Self { revision: 1, monitor: None, input_mode: InputMode::Isolated, prefer_internal_browser: true, show_cursor: true }
    }
}

pub fn initialize(app: &AppHandle) {
    let _ = APP.set(app.clone());
    let config = config_path(app).ok().and_then(|path| std::fs::read(path).ok())
...
fn config_path(app: &AppHandle) -> Result<std::path::PathBuf, String> {
    app.path().app_config_dir().map(|path| path.join("desktop-control.json")).map_err(|_| "desktop_control_storage_unavailable".into())
}

pub fn config() -> Result<DesktopControlConfig, String> {
    CONFIG.get_or_init(|| Mutex::new(DesktopControlConfig::default())).lock().map(|value| value.clone()).map_err(|_| "desktop_control_config_unavailable".into())
}
...
pub fn stop_feedback() {
    if let Some(app) = APP.get() { super::desktop_control_overlay::hide(app); }
}

pub fn monitors() -> Result<Vec<MonitorInfo>, String> {
    xcap::Monitor::all().map_err(|_| "desktop_control_monitors_unavailable")?.iter().map(super::system::monitor_info).collect()
}

pub fn selected_monitor() -> Result<MonitorInfo, String> {
    let selection = config()?.monitor.ok_or("desktop_control_monitor_required")?;
    monitors()?.into_iter().find(|monitor| monitor.id == selection.id && monitor.name == selection.name).ok_or("desktop_control_monitor_unavailable".into())
}

pub fn contains_rect(monitor: &MonitorInfo, x: i32, y: i32, width: u32, height: u32) -> bool {
    width > 0 && height > 0 && x >= monitor.x && y >= monitor.y
...
#[tauri::command]
pub fn desktop_control_save(window: super::CallerWebview, app: AppHandle, payload: DesktopControlConfig) -> Result<DesktopControlConfig, String> {
    super::ensure_main_webview(&window)?;
    let _input = super::desktop_target::lock_input()?;
    if let Some(selection) = &payload.monitor {
        if !monitors()?.iter().any(|monitor| monitor.id == selection.id && monitor.name == selection.name) {
            return Err("desktop_control_monitor_unavailable".into());
        }
    }
    let mut current = CONFIG.get_or_init(|| Mutex::new(DesktopControlConfig::default())).lock().map_err(|_| "desktop_control_config_unavailable")?;
    if current.revision != payload.revision { return Err("desktop_control_config_changed".into()); }
    let next = DesktopControlConfig { revision: current.revision.checked_add(1).ok_or("desktop_control_revision_exhausted")?, ..payload };
    let path = config_path(&app)?;
    std::fs::create_dir_all(path.parent().ok_or("desktop_control_storage_unavailable")?).map_err(|_| "desktop_control_storage_unavailable")?;
    let temporary = path.with_extension("json.tmp");
    std::fs::write(&temporary, serde_json::to_vec(&next).map_err(|_| "desktop_control_config_invalid")?).map_err(|_| "desktop_control_save_failed")?;
...
#[tauri::command]
pub async fn desktop_control_preview(window: super::CallerWebview, app: AppHandle) -> Result<(), String> {
    super::ensure_main_webview(&window)?;
...
mod tests {
    ...
    fn bounds_do_not_bridge_monitors_or_overflow() {
        let monitor = MonitorInfo { id: 1, name: "test".into(), x: -1920, y: 0, width: 1920, height: 1080, scale_factor: 1.5, primary: false };
