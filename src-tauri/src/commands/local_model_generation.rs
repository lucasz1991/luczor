...
pub(super) fn apply_sampling(body: &mut Value, artifact_hash: Option<&str>) {
    let qwen25 = matches!(artifact_hash, Some(
    ...
    let qwen35 = artifact_hash == Some("6c8c7658fe13eef22666aa89862f7fb70aa72109838cf19989e59b15875e5e08");
    // Unknown models keep their sampling distribution; the stream guard still applies.
    body["ignore_eos"] = json!(false);
    if !qwen25 && !qwen35 { return; }
    ...
    }).as_object().unwrap() { body[key] = value.clone(); }
}
...
impl RepetitionGuard {
    ...
    pub(super) fn observe(&mut self, chunk: &str) -> Option<usize> {
        for (offset, ch) in chunk.char_indices() {
            if ch == '\r' { continue; }
            if ch == '\n' {
                if self.fence.is_none() && !self.blocked_line { self.push(' '); }
                self.prefix.clear();
                self.leading = 0;
            ...
            if self.prefix.is_empty() && ch.is_whitespace() {
                ...
                if self.leading >= 4 { self.blocked_line = true; self.tail.clear(); }
            }
            if !ch.is_whitespace() && self.prefix.chars().count() < 3 {
                self.prefix.push(ch);
                if self.prefix == "```" || self.prefix == "~~~" {
                    let marker = self.prefix.chars().next().unwrap();
                    if self.fence == Some(marker) { self.fence = None; }
                    else if self.fence.is_none() { self.fence = Some(marker); }
                    self.blocked_line = true;
                    self.tail.clear();
                }
            }
            if self.fence.is_some() || self.blocked_line { continue; }
            if "`~{}[]<>=;|\\\"".contains(ch) {
                self.blocked_line = true;
                self.tail.clear();
                continue;
            ...
            if self.ticks >= 32 {
                self.ticks = 0;
                if self.repeated() { return Some(offset); }
            }
        }
        None
    }
    ...
    fn push(&mut self, ch: char) {
        if ch == ' ' && self.tail.last() == Some(&' ') { return; }
        self.tail.push(ch);
        if self.tail.len() > 4096 { self.tail.drain(..1024); }
    }

    fn repeated(&self) -> bool {
        let n = self.tail.len();
        for period in 16..=512.min(n / 6) {
            let copies = 6.max(256_usize.div_ceil(period));
            let length = copies * period;
            if length > n { continue; }
            let pattern = &self.tail[n - period..];
            if !self.tail[n - length..n - period].iter().enumerate()
                .all(|(i, ch)| *ch == pattern[i % period]) { continue; }
            if pattern.iter().filter(|c| c.is_alphabetic()).count() >= 12
                && pattern.iter().filter(|c| **c == ' ').count() >= 2 { return true; }
        }
        false
    }
}
...
pub(super) fn valid_tool_completion(
    choice: &str, definitions: &[Value], calls: &[super::NativeToolCall], finish: &str,
) -> bool {
    if (choice == "required" || finish == "tool_calls") && calls.is_empty() { return false; }
    if calls.is_empty() { return true; }
    if choice == "none" || !matches!(finish, "stop" | "tool_calls") { return false; }
    let mut ids = std::collections::HashSet::new();
    calls.iter().all(|call| {
        ids.insert(&call.id)
            && definitions.iter().any(|tool| tool.pointer("/function/name").and_then(Value::as_str)
