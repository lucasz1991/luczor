...
export function createMiniChatController(deps: Dependencies) {
  ...
  function ask(decision: MiniDecision, signal: AbortSignal): Promise<boolean> {
    ...
    return new Promise(resolve => {
      const cancel = () => { if (state.decision?.id === decision.id) decide(decision.id, false) }
  ...
  async function send(raw: string) {
    ...
    if (state.mainBusy) { state.notice = 'Der große Chat arbeitet noch. Bitte warte kurz.'; touch(); return }
    if (!state.project) { state.notice = 'Öffne zuerst ein Projekt in Luczor.'; touch(); return }
    ...
    state.notice = ''
    state.tools = []
    const user: MiniMessage = { id: crypto.randomUUID(), role: 'user', content: text, choices: [], createdAt: Date.now(), status: 'done' }
    const assistant: MiniMessage = reactive({ id: crypto.randomUUID(), role: 'assistant', content: '', choices: [], createdAt: Date.now(), status: 'running', activity: createChatActivity() })
    ...
    const toolSession: AgentToolSession = {
      queue(call) {
        if (!valid()) return
        state.tools.push({ id: call.id, name: call.name, detail: previewToolArguments(call.args, 6000), status: call.status })
        state.tools = state.tools.slice(-30)
        touch()
      },
      update(id, status) { if (!valid()) return; const tool = state.tools.find(item => item.id === id); if (tool) tool.status = status; touch() },
      approve(id) {
        const tool = state.tools.find(item => item.id === id)
        if (!valid() || !tool) return Promise.resolve(false)
        return ask({ id: crypto.randomUUID(), kind: 'tool', title: `${tool.name} ausführen?`, description: `Einmalige Freigabe für ${project.name}.`, detail: tool.detail }, current.signal)
      },
    }
    try {
      const transcript: WireMessage[] = state.messages
        .filter(message => message.id !== assistant.id && message.status === 'done')
        .map(message => ({ role: message.role, content: [message.content, message.question, ...message.choices].filter(Boolean).join('\n') }))
      const baseMessages: WireMessage[] = [
        { role: 'system', content: deps.preamble(state.mode, project.name) + '\nTemporärer Mini-Chat: keine Unterhaltung automatisch als Memory oder Projektzusammenfassung speichern. Antworte kurz. Bei einer Rückfrage kannst du ein JSON-Objekt mit summary, question und maximal vier kurzen Antwortoptionen in bullets ausgeben. Eine Antwortoption ist keine Tool-Freigabe.' },
        ...compactHistory(normalizeConversationHistory(transcript), 2400),
      ]
      const result = await deps.run({
        projectId: project.id, mode: state.mode, getMode: () => deps.context().mode,
        baseMessages, externalBaseMessages: baseMessages, contextEgress: 'external_allowed',
        signal: current.signal, toolSession,
        onProgress(event) { if (valid() && assistant.activity) { updateChatActivity(assistant.activity, event); touch() } },
        onToken(content) { if (valid()) { const parsed = parseEnvelope(content); assistant.content = (parsed?.summary || content).slice(0, 16_000); touch() } },
        requestExternalApproval(summary) {
          if (!valid()) return Promise.resolve(false)
          return ask({ id: crypto.randomUUID(), kind: 'external', title: 'Externes Modell einmalig verwenden?', description: 'Das lokale Modell ist für diese Anfrage nicht verfügbar.', detail: `${summary.messageCount} Nachrichten · ${summary.characterCount} Zeichen\nZiel: ${summary.destination}\nOhne Tools · Paket ${summary.packetHash.slice(0, 16)}` }, current.signal)
        },
      })
      if (!valid()) return
      ...
      assistant.content = current.signal.aborted ? 'Abgebrochen.' : `Die Anfrage konnte nicht abgeschlossen werden. ${error instanceof Error ? error.message : 'Bitte erneut versuchen.'}`.slice(0, 1800)
      if (assistant.activity) finishChatActivity(assistant.activity, assistant.status)
    } finally {
      if (state.sessionId === sessionId && abort === current) {
        if (current.signal.aborted) {
          ...
          state.tools.forEach(tool => { if (['proposed', 'approved', 'executing'].includes(tool.status)) tool.status = 'canceled' })
        }
        state.busy = false
        state.decision = null
        abort = null
  ...
  function dispatch(action: MiniAction) {
    if (action.type === 'ready') { refresh(); return }
    if (action.type === 'mode') { deps.setMode(action.mode); refresh(); return }
    if (action.type === 'main_decide' || action.type === 'kill_switch') return
    if (action.sessionId !== state.sessionId) return
    switch (action.type) {
      case 'send': return send(action.text)
      case 'stop': stop(); break
      case 'reset': reset(); break
      case 'decide': decide(action.id, action.approved); break
    }
  }
  refresh()
