...
export function miniWorkflowBudgetSnapshot(run: WorkflowRun): WorkflowRun {
  ...
  for (const row of budget.rows) {
    if (row.key === 'executions') {
      policy.max_executions = row.limit
      state.executions = row.used
    } else if (row.key === 'active') {
      policy.active_seconds = Math.round(row.limit * 60)
      state.active_ms = Math.round(row.used * 60000)
    }
  }
  ...
  const snapshot: WorkflowRun = {
    ...
    project_external_id: run.project_external_id,
    status: run.status.slice(0, 64),
    sandbox: run.sandbox,
  }
  return Object.assign(
    ...
    budget.inherited
      ? {
          ...
          ...(!budget.rootUnavailable
            ? { root_budget: { id: budget.rootId, status: budget.rootStatus, budgets: policy, budget_state: state } }
            : {}),
        }
