...
export function researchProbeMutationKey(key: string): boolean {
  try {
    ...
    return Array.isArray(parsed) && parsed.length === 2 && typeof parsed[0] === 'string' && isResearchProbe(parsed[0]) &&
...
export function researchOnlyUncertainty(checkpoint?: AgentCheckpoint): boolean {
  if (!checkpoint ||
    checkpoint.pendingTaskCreateVerifications?.some(item => item.state !== 'verified_present')) return false
  const uncertain = checkpoint.uncertainMutations ?? []
  const unresolved = unresolvedCheckpointCalls(checkpoint.messages)
  return uncertain.length + unresolved.length > 0 && uncertain.every(researchProbeMutationKey) &&
    unresolved.every(call => isResearchProbe(call.function.name))
}
