...
import { refreshWorkflowCapabilities } from './capabilities'
...
export const workflowTestModeLabel = (mode: WorkflowTestMode) =>
  mode === 'definition' ? 'Definitionsprüfung' : mode === 'simulation' ? 'Simulation' : 'Echter Gerätetest'
export function workflowTestResultLabel(test: Pick<WorkflowTestEvidence, 'mode' | 'status'>): string {
  ...
  return (
    (
      { running: 'Prüfung läuft', failed: 'Prüfung fehlgeschlagen', cancelled: 'Abgebrochen' } as Record<string, string>
...
export function createWorkflowTests(projectId: string, signal: AbortSignal) {
  const connect = async (workflowId: number, mutating: boolean) => {
    ...
    const owned = <T extends { workflow_definition_id: number }>(item: T): T => {
      if (item.workflow_definition_id !== workflowId)
        throw new Error('Die Testdaten gehören zu einem anderen Workflow.')
      return item
    }
    ...
    const write = <T>(path: string, args: Record<string, unknown>, expectedVersion?: number) => {
      ...
      return workflowOperations.run<T>({
        ...
        async verify(operationId) {
          const result = await request<{ status: string; response?: T }>(
            `/workflow-operations/${encodeURIComponent(operationId)}`
          )
        ...
        async execute(operationId) {
          if (expectedVersion !== undefined && (await access.workflow(workflowId)).version !== expectedVersion)
            throw Object.assign(new Error('Die Workflow-Version wurde geändert. Bitte den aktuellen Stand laden.'), {
              status: 409,
            })
          await access.check()
          const result = await requestWithConfig<{ data: T }>(
            ...
            {
              method: 'POST',
              body: { ...args, operation_id: operationId },
              signal: access.execution.signal,
            },
            access.config
          )
          await access.check()
  ...
  return {
    async load(workflowId: number): Promise<WorkflowTestState> {
      ...
      cases.forEach(ctx.owned)
      tests.forEach(ctx.owned)
      repairs.forEach(ctx.owned)
    ...
    async createCase(workflowId: number, name: string, specification: Record<string, unknown>, authorizeReal: boolean) {
      ...
      if (authorizeReal) {
        const preview = JSON.stringify(
          {
            ...
            device: ctx.access.config.clientId,
            specification: spec,
          },
          null,
          2
        ...
        await ctx.confirm(
          `Diesen unveränderlichen Testfall für echte Tests auf diesem Gerät freigeben? Einzelne Dateizugriffe, Skripte und andere Effekte behalten ihre bestehenden Freigaben.\n\n${preview}`
        )
        spec.real_test_authorized = true
        spec.device_id = ctx.access.config.clientId
      }
      return ctx.owned(
        await ctx.write<WorkflowTestCase>(`/workflows/${workflowId}/test-cases`, { name, specification: spec })
      )
    },
    async start(
      workflowId: number,
    ...
    async configureRepair(
      ...
      for (const key of [
        ...
        if (Reflect.get(policy, key) !== Reflect.get(body, key))
          throw new Error('Die gespeicherte Reparaturfreigabe weicht von der bestätigten Auswahl ab.')
      }
    ...
    async propose(workflowId: number, expectedVersion: number, sourceRunId: number, definition: WorkflowDefinition) {
      const ctx = await connect(workflowId, true)
      return ctx.owned(
        await ctx.write<WorkflowRepair>(
          `/workflows/${workflowId}/repairs`,
          { source_run_id: sourceRunId, expected_version: expectedVersion, definition },
          expectedVersion
        )
      )
    },
    async activate(workflowId: number, repairId: number) {
      const ctx = await connect(workflowId, true)
      const repair = (await ctx.request<WorkflowRepair[]>(`/workflows/${workflowId}/repairs`)).find(
        item => item.id === repairId
      )
      if (!repair || ctx.owned(repair).status !== 'proposed')
        throw new Error('Der Reparaturentwurf ist nicht mehr verfügbar.')
      await ctx.confirm(
        `Geprüften Reparaturentwurf ${repairId} für künftige Läufe von „${ctx.workflow.name}“ aktivieren? Der Server prüft die echte Testevidenz und den unveränderten Freigaberahmen erneut.`
      )
