...
const dependencies: LlmDependencies = {
  ...
  external: async (request, ticket) => {
    ...
    const approved = await requestPayloadApproval(
      {
        title: 'Workflow: einmalige externe KI-Anfrage',
        kind: 'inference',
        destination: `${account.config.baseUrl}/api/v1/proxy/chat`,
        hash,
        content,
      },
      ticket.signal
    ...
    if (
      ...
      current.config.deviceKey !== account.config.deviceKey ||
      current.config.baseUrl !== account.config.baseUrl ||
      current.config.clientId !== account.config.clientId
    )
...
export async function runWorkflowLlm(
  input: WorkflowLlmInput,
  context: LlmContext,
  ...
  if (input.inference && !['local', 'external'].includes(input.inference))
    throw new Error('workflow_llm_inference_invalid')
  if (input.output_format && !['text', 'json'].includes(input.output_format))
    throw new Error('workflow_llm_output_format_invalid')
  ...
  const timer = setTimeout(
    () => controller.abort(new Error('workflow_llm_timeout')),
    Math.max(5, Math.min(600, input.timeout_seconds ?? 120)) * 1000
  )
  ...
  try {
    ...
    if (input.output_format === 'json') {
      try {
        ...
        throw new Error('workflow_llm_invalid_json')
      }
    ...
    return {
      ...
      usage_source: result.usage ? 'reported' : 'unavailable',
    }
  } finally {
    clearTimeout(timer)
  }
