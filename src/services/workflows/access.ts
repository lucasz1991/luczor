...
export async function captureWorkflowAccess(ctx: ToolContext, requestedProjectId: unknown, mutating: boolean) {
  ...
  const scope = ctx.workspaceScope ? { principalId: ctx.workspaceScope.principalId, ids: [...ctx.workspaceScope.projectIds] } : undefined
  const assert = () => {
    executionGate.assert(execution, mutating)
    ctx.signal?.throwIfAborted()
    if (!projectId || !state.projects.some(project => project.id === projectId && !project.archivedAt)) throw new Error('Ein verfügbares Zielprojekt ist erforderlich.')
    if (scope) {
      if (!requestedProjectId || ctx.inferenceTarget !== 'local' || !scope.ids.includes(projectId)) throw new Error('Das Workflow-Zielprojekt muss ausdrücklich zum freigegebenen Arbeitsbereich gehören.')
  ...
  const check = async () => {
    ...
    assert()
    if (current.baseUrl !== config.baseUrl || current.deviceKey !== config.deviceKey || current.clientId !== config.clientId) throw new Error('Die Workflow-Kontoverbindung wurde geändert.')
    if (scope && await resolveWorkspacePrincipalId() !== scope.principalId) throw new Error('Das Workflow-Konto wurde geändert.')
    assert()
  }
  await check()
  ...
  const workflow = async (id: number): Promise<Workflow> => {
    ...
    await check()
    if (response.data.project_external_id !== projectId) throw new Error('Der Workflow gehört zu einem anderen Projekt.')
