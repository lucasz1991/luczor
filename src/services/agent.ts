...
export function shouldRequireToolCall(text: string): boolean {
  ...
  const imperative = /(?:^|[^\p{L}\p{N}_])(speichere|erstelle|lege|setze|ändere|aktualisiere|lösche|markiere|prüfe|kontrolliere|öffne|klicke|schreibe|führe|starte|stoppe)(?=$|[^\p{L}\p{N}_])/u;
  const infinitive = /(?:^|[^\p{L}\p{N}_])(speichern|erstellen|anlegen|setzen|ändern|aktualisieren|löschen|markieren|prüfen|kontrollieren|öffnen|klicken|schreiben|ausführen|starten|stoppen)(?=$|[^\p{L}\p{N}_])/u;
  const requestCue = /(?:^|[^\p{L}\p{N}_])(bitte|jetzt|nun|sollst du|du sollst|kannst du|mach|mache|ok dann)(?=$|[^\p{L}\p{N}_])/u;
  return imperative.test(normalized) || (infinitive.test(normalized) && (requestCue.test(normalized) || normalized.split(/\s+/).length <= 4));
}
...
export function looksLikeInternalReasoningLeak(text: string): boolean {
  ...
  return /^(?:analysis\s*:|internal reasoning\s*:|we need to (?:respond|answer|set|use|check|call|determine)|the user (?:says|asks|wants)|according to (?:the )?(?:system|developer))/i.test(start);
}
...
export async function runAgent(opts: RunAgentOptions): Promise<{ finalText: string; requestId?: string; model?: string; provider?: string; useCase?: string; toolFailures: number; toolSuccesses: number }> {
  ...
  for (let round = 0; round < maxRounds; round++) {
    ...
    if (!res.toolCalls.length) {
      ...
      return { finalText, requestId: lastRequestId, model: lastModel, provider: lastProvider, useCase: lastUseCase, toolFailures, toolSuccesses };
    }
