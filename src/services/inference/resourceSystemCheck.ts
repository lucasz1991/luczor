...
export function resourceSystemCheck(hardware: HardwareSnapshot, config: LocalResourceConfig) {
  ...
  if (
    ...
    available < 1 ||
    !bytes(hardware.memory.totalBytes) ||
    !bytes(hardware.memory.availableBytes) ||
    hardware.memory.totalBytes < 2 * GIB
  )
  ...
  const gpus = hardware.accelerators
    ...
    .map(gpu => {
      ...
      const free =
        bytes(gpu.availableBytes) && bytes(gpu.totalBytes) ? Math.min(gpu.availableBytes, gpu.totalBytes) : null
  ...
  return {
    capturedAtMs: hardware.capturedAtMs,
    maximumThreads,
    maximumRam,
    ramReserve,
...
export function percentageResourceConfig(
  hardware: HardwareSnapshot,
  config: LocalResourceConfig,
  limits: ResourcePercentageLimits
): LocalResourceConfig {
  const next = { ...config, percentageLimits: { ...limits } }
  const check = resourceSystemCheck(hardware, next)
  return {
    ...next,
    threads: check.responseThreads,
    threadsBatch: check.contextThreads,
...
export function sameResourceIntent(first: LocalResourceConfig, second: LocalResourceConfig): boolean {
  ...
  return (
    ...
    JSON.stringify(first.gpuDeviceIds) === JSON.stringify(second.gpuDeviceIds) &&
