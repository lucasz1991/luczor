...
function normalize(o: any): { summary: string; question: string; bullets: string[] } {
  const summary =
    typeof o?.summary === "string" ? o.summary : typeof o?.content === "string" ? o.content : "";
  const question = typeof o?.question === "string" ? o.question : "";
  const bullets = Array.isArray(o?.bullets)
    ? o.bullets.filter((x: any) => typeof x === "string").map((x: string) => x.trim()).filter(Boolean)
    : [];
...
function extractStringField(t: string, key: string): string | null {
