...
const palette = computed(() => {
  switch (hud.status) {
    case "listening": return { main: "#22d3ee", accent: "#67e8f9", glow: "rgba(34,211,238,.65)", label: "Höre zu" };
    case "thinking":  return { main: "#38bdf8", accent: "#7dd3fc", glow: "rgba(56,189,248,.6)",  label: "Denke nach" };
    case "executing": return { main: "#f59e0b", accent: "#fcd34d", glow: "rgba(245,158,11,.72)", label: "Führe aus" };
    case "speaking":  return { main: "#34d399", accent: "#6ee7b7", glow: "rgba(52,211,153,.62)", label: "Spreche" };
    case "error":     return { main: "#f43f5e", accent: "#fda4af", glow: "rgba(244,63,94,.72)",  label: "Fehler" };
    default:          return { main: "#38bdf8", accent: "#7dd3fc", glow: "rgba(56,189,248,.4)",  label: "Bereit" };
  }
});
...
const spectrumLines = computed(() => {
  ...
  return arr.map((v, i) => {
    ...
    return {
      ...
      x2: cx + (r0 + len) * cos, y2: cy + (r0 + len) * sin,
      o: 0.35 + v * 0.6,
    };
  });
});
...
const ticks = computed(() => {
  ...
  return Array.from({ length: 60 }, (_, i) => {
    ...
    const long = i % 5 === 0;
    const rOut = 90, rIn = long ? 80 : 85;
    return {
      x1: 100 + rOut * Math.cos(ang), y1: 100 + rOut * Math.sin(ang),
      x2: 100 + rIn * Math.cos(ang),  y2: 100 + rIn * Math.sin(ang),
      o: long ? 0.8 : 0.4,
    };
  });
});
...
const segments = computed(() => {
  ...
  return Array.from({ length: SEGMENTS }, (_, i) => {
    ...
    const on = i < lit;
    return {
      x1: 100 + 66 * Math.cos(ang), y1: 100 + 66 * Math.sin(ang),
  ...
  <div class="jarvis" :class="{ collapsed, embedded: props.embedded }" :style="posStyle">
    ...
    <transition name="hud-fade">
      <div v-if="!collapsed" class="jarvis-body">
        ...
        <div class="reactor" :style="{ filter: `drop-shadow(0 0 20px ${palette.glow})` }">
          <svg viewBox="0 0 200 200" width="190" height="190">
            ...
            <g :transform="`rotate(${rotOuter} 100 100)`" :stroke="palette.main" stroke-width="1.5" class="tint">
              <line v-for="(tk, i) in ticks" :key="'t'+i" :x1="tk.x1" :y1="tk.y1" :x2="tk.x2" :y2="tk.y2" :opacity="tk.o" />
            </g>

            <!-- mid dashed counter-rotating ring -->
            <circle :transform="`rotate(${rotMid} 100 100)`" cx="100" cy="100" r="60"
              fill="none" :stroke="palette.main" stroke-width="2.5" stroke-dasharray="5 13" opacity="0.75" class="tint" />

            <!-- segmented energy ring -->
            <g stroke-linecap="round" filter="url(#softGlow)">
              <line v-for="(s, i) in segments" :key="'s'+i" :x1="s.x1" :y1="s.y1" :x2="s.x2" :y2="s.y2"
                :stroke="s.on ? palette.accent : 'rgba(255,255,255,.10)'"
                :stroke-width="s.on ? 3 : 2" :opacity="s.on ? 0.95 : 1" class="tint seg" />
            </g>

            <!-- activity arcs (net / file / os) -->
            <g filter="url(#softGlow)">
              <circle :transform="`rotate(${rotArcs} 100 100)`" cx="100" cy="100" r="50" fill="none"
                stroke="#22d3ee" :stroke-width="arc(sNet).width" :opacity="arc(sNet).opacity"
                stroke-dasharray="46 210" stroke-linecap="round" />
              <circle :transform="`rotate(${rotArcs + 120} 100 100)`" cx="100" cy="100" r="50" fill="none"
                stroke="#a78bfa" :stroke-width="arc(sFile).width" :opacity="arc(sFile).opacity"
                stroke-dasharray="46 210" stroke-linecap="round" />
              <circle :transform="`rotate(${rotArcs + 240} 100 100)`" cx="100" cy="100" r="50" fill="none"
                stroke="#f59e0b" :stroke-width="arc(sOs).width" :opacity="arc(sOs).opacity"
                stroke-dasharray="46 210" stroke-linecap="round" />
            </g>

            <!-- circular audio spectrum -->
            <g :stroke="palette.main" stroke-width="1.6" stroke-linecap="round" filter="url(#softGlow)" class="tint">
              <line v-for="(l, i) in spectrumLines" :key="'sp'+i" :x1="l.x1" :y1="l.y1" :x2="l.x2" :y2="l.y2" :opacity="l.o" />
            </g>

            <!-- expanding ping -->
            <circle cx="100" cy="100" :r="pingR" fill="none" :stroke="palette.accent"
              stroke-width="1.5" :opacity="pingOpacity" class="tint" />
        ...
        <!-- mirrored waveform -->
        <div class="wave">
          <span v-for="(h, i) in bars" :key="i" class="bar"
            :style="{
              height: (6 + h * 54) + 'px',
        ...
        <!-- channel meters -->
        <div class="meters">
          <div class="meter"><span class="tag">NET</span>
            <span class="track"><i :style="{ width: (sNet*100)+'%', background:'linear-gradient(90deg,#0891b2,#22d3ee)' }" /></span></div>
          <div class="meter"><span class="tag">FILE</span>
            <span class="track"><i :style="{ width: (sFile*100)+'%', background:'linear-gradient(90deg,#7c3aed,#a78bfa)' }" /></span></div>
          <div class="meter"><span class="tag">OS</span>
...
.status-label, .dot { transition: color .55s ease, background .55s ease, box-shadow .55s ease, text-shadow .55s ease; }
...
.jarvis-body {
  ...
  overflow: hidden;
  background:
    radial-gradient(120% 90% at 50% 0%, rgba(34,211,238,.08), transparent 60%),
    linear-gradient(160deg, rgba(6,18,28,.9), rgba(2,8,14,.94));
  border: 1px solid rgba(56,189,248,.28);
  border-radius: 20px;
  padding: 14px 13px 11px;
  backdrop-filter: blur(14px);
  box-shadow: 0 16px 48px rgba(0,0,0,.55), inset 0 0 34px rgba(34,211,238,.06);
}
...
.kill-banner {
  ...
  color: #fecaca; background: rgba(244,63,94,.16);
  border: 1px solid rgba(244,63,94,.5); border-radius: 9px;
  padding: 3px 6px; margin-bottom: 8px;
  animation: blink 1.1s steps(2) infinite;
}

.reactor { position: relative; display: flex; flex-direction: column; align-items: center; margin-bottom: 4px; }
.seg { transition: stroke .25s ease, stroke-width .25s ease; }
.status-label {
  margin-top: -8px; font-size: 12px; letter-spacing: .22em; text-transform: uppercase; font-weight: 600;
}

.wave {
  display: flex; align-items: center; justify-content: center; gap: 2px;
  height: 60px; margin: 8px 2px 12px;
}
.wave .bar {
...
.meters { display: grid; gap: 6px; margin-bottom: 9px; }
.meter { display: flex; align-items: center; gap: 8px; font-size: 10px; }
.meter .tag { width: 32px; color: #86bfd6; letter-spacing: .12em; }
.meter .track {
  flex: 1; height: 6px; border-radius: 5px;
...
.kill:hover { background: rgba(16,185,129,.2); }
.kill.on {
  color: #fff; background: #e11d48; border-color: #e11d48;
  box-shadow: 0 0 16px rgba(225,29,72,.65);
}

.shot { margin-top: 9px; border-radius: 10px; overflow: hidden; border: 1px solid rgba(56,189,248,.28); }
.shot img { display: block; width: 100%; }

.hud-fade-enter-active, .hud-fade-leave-active { transition: opacity .3s ease, transform .3s ease; }
.hud-fade-enter-from, .hud-fade-leave-to { opacity: 0; transform: translateY(8px) scale(.97); }
.shot-fade-enter-active { transition: opacity .4s ease, max-height .4s ease; }
.shot-fade-enter-from { opacity: 0; }
...
@media (prefers-reduced-motion: reduce) {
  .scanline, .jarvis-toggle .dot, .kill-banner { animation: none; }
}
</style>
