...
export function useAutonomousGoal(input: {
  ...
  const persist = async (id: string, next: GoalRunState, expectedRevision: number, text?: string) => {
    ...
    if (disposed || identity !== identityGeneration || !latest || (latest.autonomousGoal?.revision ?? 0) !== expectedRevision) {
  ...
  const controller = createAutonomousGoalController({
    ...
    persist,
    canRun: id =>
      !disposed && !error.value && isTauri() && id === input.projectId() && input.available() && !input.draft().trim() &&
      !!current(id) && !current(id)?.archivedAt && current(id)?.goal === current(id)?.autonomousGoal?.text,
    ...
    onPersistenceError: (_id, cause) => {
      error.value = cause instanceof Error ? cause.message : 'Ziel konnte nicht gesichert werden. Bitte erneut aktivieren.'
    },
  })
  ...
  const save = (text: string) => guarded(async () => {
    text = text.trim()
    ...
    const next: GoalRunState = {
      text, active: false, status: 'idle', revision: revision + 1, iterations: 0,
      phase: 'work', updatedAt: Date.now(),
    }
    if (!await persist(id, next, revision, text)) throw new Error('Das Projekt wurde gewechselt. Ziel bitte erneut speichern.')
  })
  const toggle = (active: boolean) => guarded(async () => {
    const id = input.projectId()
    if (!active) { await controller.stop(id, 'Von dir pausiert.'); return }
    ...
    const next: GoalRunState = {
      ...previous, active: true, status: 'waiting', revision: previous.revision + 1,
      phase: previous.status === 'completed' ? 'work' : previous.phase,
      reason: undefined, stagnantIterations: 0, consecutiveErrors: 0, updatedAt: Date.now(),
    }
  ...
  const identityChanged = () => {
    ...
    for (const project of state.projects) {
      ...
      project.autonomousGoal = { ...goal, active: false, status: 'waiting', revision: goal.revision + 1,
        reason: 'Konto gewechselt. Ziel erneut aktivieren.', updatedAt: Date.now() }
    }
    void guarded(async () => {
  ...
  watch(() => [input.projectId(), input.available(), input.draft(), model.value?.active, model.value?.revision], () => {
  ...
  watch(() => current(input.projectId())?.goal, text => {
    if (model.value?.active && text !== model.value.text)
      void guarded(() => controller.stop(input.projectId(), 'Das Projektziel wurde geändert. Zieltext prüfen und erneut speichern.'))
  })
  onBeforeUnmount(() => {
    disposed = true
    controller.dispose()
