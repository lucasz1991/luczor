...
function ensurePendingBucket(projectId: T.Id): T.PendingToolCall[] {
  ...
  const existing = state.pending.toolCallsByProject[projectId];
  if (existing) return existing;

  const bucket: T.PendingToolCall[] = [];
  state.pending.toolCallsByProject[projectId] = bucket;
  return bucket;
}
...
export const mutations = {
  ...
  hydrate(next: T.AppState) {
    ...
    state.pending = fresh.pending ?? ({ toolCallsByProject: {} } as T.PendingState);
    state.pending.toolCallsByProject ??= {};
  ...
  ensureDefaults() {
    ...
    const hasAnyVisible = state.messages.some(
      (m) => m.projectId === pid && m.visibility !== "hidden"
    );

    if (!hasAnyVisible) {
      state.messages.push(
        makeMsg("assistant", "Willkommen. Was ist das Ziel dieses Projekts?", pid)
      );
    }
  },
  ...
  addProject(p: { id: T.Id; name: string }) {
    ...
    const hasAnyVisible = state.messages.some(
      (m) => m.projectId === p.id && m.visibility !== "hidden"
    );

    if (!hasAnyVisible) {
      state.messages.push(
        makeMsg("assistant", "Willkommen. Was ist das Ziel dieses Projekts?", p.id)
      );
    }
  },
  ...
  resetProjectChat(projectId: T.Id) {
    ...
    const hidden = state.messages.filter(
      (m) => m.projectId === projectId && m.visibility === "hidden"
    );
  ...
  queueToolCall(
    projectId: T.Id,
    call: Omit<T.PendingToolCall, "projectId" | "createdAt" | "updatedAt">
  ) {
    ensureProjectExists(projectId);
